    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Create, assign, and cover shifts</p>
        </div>
      ...
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-4 lg:grid-cols-7">
        {weekDays.map((d) => {
          ...
          return (
            <div key={key} className={`rounded-xl border p-3 text-center ${isToday ? "border-primary/40 bg-primary/5" : "border-border/60 bg-white/70"}`}>
      ...
      <Card className="rounded-2xl border-border/60 bg-white/80 shadow-sm backdrop-blur">
        <CardHeader>
          <CardTitle className="flex items-center gap-2 text-lg"><Calendar className="h-5 w-5 text-primary" /> Upcoming shifts</CardTitle>
          <CardDescription>Assign coverage from your roster</CardDescription>
        </CardHeader>
        <CardContent>
          {loading ? (
            ...
            <ul className="divide-y divide-border/60">
              {shifts.map((s) => (
                <li key={s.id} className="flex flex-col gap-3 py-4 first:pt-0 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    <div className="flex flex-wrap items-center gap-2">
                      <span className="font-medium">{s.role}</span>
                      <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${s.status === "open" ? "bg-amber-100 text-amber-800" : "bg-emerald-100 text-emerald-800"}`}>{s.status}</span>
                    </div>
                    <p className="mt-1 text-sm text-muted-foreground">{formatWhen(s.startTime)} → {formatWhen(s.endTime)}</p>
                    <div className="mt-1 flex flex-wrap gap-3 text-xs text-muted-foreground">
                      {s.location && <span className="inline-flex items-center gap-1"><MapPin className="h-3 w-3" />{s.location.name}{s.location.city ? ` · ${s.location.city}` : ""}</span>}
                      <span className="inline-flex items-center gap-1"><User className="h-3 w-3" />{s.user?.name || "Unassigned"}</span>
                    </div>
                  </div>
                  {(!s.user || s.status === "open") && team.length > 0 && (
                    <select className="rounded-lg border px-2 py-1.5 text-sm" defaultValue="" disabled={assigning === s.id}
